...
namespace ChapterMaster
{
    class SectorRenderer
    {
        ...
        {
            Pixel = new Texture2D(GameState.graphicsDevice, 1, 1);
        ...
        public void DrawLine(Vector2 start, Vector2 end, Color color, ViewController view)
        {
            ...
            Vector2 originTransform = new Vector2(GameState.GetWidth() / 2, GameState.GetHeight() / 2);
        ...
        public void DrawStar(SpriteBatch spriteBatch, World.System system, Color color, ViewController view)
        {
            ...
            spriteBatch.Draw(GameState.SystemTextures[system.color],rect, Color.White);
            ...
            Vector2 nameSize = GameState.Courier_New.MeasureString(name);
            // TODO: scale system name better.
            spriteBatch.DrawString(GameState.Courier_New, name, position, Color.White, 0, new Vector2(0,0),view.zoom+0.3f, SpriteEffects.None,0);
            string owners = "";
            ...
            spriteBatch.DrawString(GameState.Courier_New, owners, new Vector2(rectLabel.Left, rectLabel.Bottom + nameSize.Y * view.zoom + 1), Color.White, 0, new Vector2(0, 0), view.zoom + 0.3f, SpriteEffects.None, 0);
        }
        public void DrawFleet(SpriteBatch spriteBatch, Fleet.Fleet fleet, Color color, ViewController view, Sector sector)
        {
            ...
            if (!fleet.isMoving)
            {
                ...
                for (int orbitingFleetId = 0; orbitingFleetId < orbitingFleets.Count; orbitingFleetId++)
                {
                    ...
                    spriteBatch.Draw(GameState.FleetTextures[currentFleet.fleetFaction][currentFleet.fleetState],
                    ...
                    if (orbitingFleets[orbitingFleetId].isSelected)
                    {
                        primitive.Circle(
                            new Vector2((sector.Systems[fleet.originSystemId].x + 40 + 30 + (Constants.SystemSize / 2) * orbitingFleetId - view.camX)
                                        * view.zoom + GameState.GetWidth() / 2,
                                        (sector.Systems[fleet.originSystemId].y + 30 - 20 - view.camY)
                                        * view.zoom + GameState.GetHeight() / 2),
            ...
            else
            {
                ...
                Rectangle rect = view.TransformedOriginRect(Position.X,
                                                            ...
                                                            Constants.SystemSize,
                                                            true);
                spriteBatch.Draw(GameState.FleetTextures[fleet.fleetFaction][fleet.fleetState], rect, Color.White);
                ...
                DrawLine(oSystem + new Vector2(Constants.SystemSize / 2,
                         ...
                         Position + new Vector2(Constants.SystemSize / 2,
                                                Constants.SystemSize / 2),
                        trailColor, view);
                spriteBatch.DrawString(GameState.ARJULIAN, "ETA: " + (travelTurns - fleet.fleetMoveProgress), new Vector2(rect.Location.X+5,rect.Location.Y-5), Color.White);
